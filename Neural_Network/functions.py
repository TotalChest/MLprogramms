...
def nnCostFunction(nn_params, layers, X, Y, lambda_=0.0):
	...
	for i in range(m):
		J += (-1 / m) * np.sum(np.log(feedforward(X[i,:], Theta, len(Theta))) * Y1[i,:] + np.log(1-feedforward(X[i,:], Theta, len(Theta))) * (1- Y1[i,:]))
	...
	for i in range(m):
		...
		for j in range(len(Theta)-1):
			d.insert(0, np.dot(Theta[len(Theta)-j-1].T, d[0][1:]) * a[len(Theta)-j-1] * (1 - a[len(Theta)-j-1]))
