...
class KNNClassifier:
    ...
    def __init__(self, n_neighbors, algorithm='my_own',
                 metric='euclidean', weights='uniform'):
        if algorithm == 'my_own':
            finder = NearestNeighborsFinder(n_neighbors=n_neighbors,
                                            metric=metric)
        elif algorithm in ('brute', 'ball_tree', 'kd_tree',):
            finder = NearestNeighbors(n_neighbors=n_neighbors,
                                      algorithm=algorithm,
                                      metric=metric)
        else:
    ...
    def _predict_precomputed(self, indices, distances):
        minlength = np.max(self._labels) + 1
        nearest_labels = np.take(self._labels, indices)
        if self._weights == 'uniform':
            frec_labels = np.apply_along_axis(
                lambda x: np.bincount(x, minlength=minlength),
                1, nearest_labels
                )
        else:
            n = indices.shape[1]
            ...
            frec_labels = np.apply_along_axis(
                lambda x: np.bincount(x[:n].astype('int64'),
                                      weights=x[n:],
                                      minlength=minlength),
                1, np.hstack((nearest_labels, weights))
                )
        labels = np.argmax(frec_labels, axis=1)
        return labels
...
class BatchedMixin:
    ...
    def kneighbors(self, X, return_distance=False):
        ...
        for i_min in range(0, X.shape[0], batch_size):
            ...
            indices_ = super().kneighbors(X_batch,
